...
def add_simple_list(pdf_elements, modules, title):
    ...
    items = [ListItem(Paragraph(f" {title}<br/>", styles['BodyText'])) for i, title in enumerate(modules)]
...
def add_two_elements_list(pdf_elements, actors, title, name_one, name_two):
    ...
    items = [ListItem(Paragraph(f" <b>{element[name_one]}</b>: {element[name_two]}<br /><br />", styles['BodyText'])) for i, element in enumerate(actors)]
    pdf_elements.append(ListFlowable(items, bulletType='bullet'))
...
def generate_pdf(project):
    ...
    with tempfile.NamedTemporaryFile(suffix='.png', delete=True) as tmp:
        ...
        if project['created_at']:
            add_simple_text(pdf_elements, "Created at", project['created_at'].strftime("%Y-%m-%d %H:%M:%S.%f"))

        if project['title']:
            add_simple_list(pdf_elements, project['title']['names'], "Titles")

        if project['elevator_speech']:
            add_simple_text(pdf_elements, "Elevator speech", project['elevator_speech']['content'])
        ...
        if project['database_schema']:
            add_er_diagram(pdf_elements, parse_database_to_erdiagram_mermaid(project['database_schema']), tmp.name, "Database Schema")

        if project['actors']:
            add_two_elements_list(pdf_elements, project['actors']['actors'], "Actors", "name", "description")

        if project['specifications']:
            add_two_elements_list(pdf_elements, project['specifications']['specifications'], "Specifications", "name", "description")

        if project['risks']:
            add_three_element_list(pdf_elements, project['risks']['risks'], "Risks", "risk", "description", "prevention")

        if project['requirements'] and project['requirements']['functional_requirements']:
            add_three_element_list(pdf_elements, project['requirements']['functional_requirements'], "Functional Requirements", "name", "description", "priority")

        if project['requirements'] and project['requirements']['non_functional_requirements']:
            add_three_element_list(pdf_elements, project['requirements']['non_functional_requirements'], "Non Functional Requirements", "name", "description", "priority")

        if project['project_schedule']:
            add_three_element_list(pdf_elements, project['project_schedule']['milestones'], "Project Schedule", "name", "description", "duration")

        if project['business_scenarios']:
            add_two_elements_list(pdf_elements, project['business_scenarios']['business_scenario']['features'], "Business Scenarios", "feature_name", "description")
