...
def update_component_task(
    project_id: str, query: str, ai_model: AI, get_project_dao_ref, generate_component_class: type(Generate)
):
    """
    ...
    try:
        _, err = ray.get(update_component.fetch_from_database.remote(get_project_dao_ref, project_id))
        if err:
            raise err
        ...
        _, err = ray.get(update_component.save_to_database.remote(get_project_dao_ref, project_id))
        if err:
            raise err
