...
class Module(metaclass=abc.ABCMeta):
    ...
    """
    def __init__(self, model_class, what, expected_format, component_identify: ComponentIdentify):
        """
    ...
    def fetch_from_database(self, project_id):
        """Fetch the model from project with id={project_id} from the database.
           returns the actor ref and an error if any."""
        try:
            self.value = projects_dao.get_project_component(project_id, self.component_identify.value)
