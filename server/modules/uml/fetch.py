...
async def fetch_uml_list(ai_call_func, is_mock=False):
    ...
    try:
        ...
        try:
            diagramsListJson = json.load(open( os.path.join(dirname,'data','test','diagramsListResponse.json'),encoding='utf8')) if is_mock else json.loads(ai_call_func(diagrams_query, "json"))
            if not validate_json(diagramsListJson,required_schema ) : return None
    ...
    if diagramsListJson is not None:
        try:
            for actor, data in diagramsListJson['actors'].items():
                ...
                text_response_for_uml_code = read_from_file(os.path.join(dirname,'data','test','umls', f"{actor}.uml")) if is_mock else  ai_call_func(uml_query, None)
